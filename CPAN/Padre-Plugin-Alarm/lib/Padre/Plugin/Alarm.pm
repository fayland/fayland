...
sub set_alarm_time {
	...
	my @layout = (
		...
		[
			...
			[ 'Wx::ComboBox',   '_frequency_',     '', \@frequency],
		],
		[
...
sub _set_alarm {
	...
	unless ( $timer->IsRunning ) {
		...
		$timer->Start(60 * SECONDS, 0); # every minute
	}
}
...
sub on_timer_alarm {
	...
	# get now-time;
	my @ntime = localtime();
	my $ntime = sprintf('%d:%2d', $ntime[2], $ntime[1]);
	...
	foreach ( @$config ) {
		...
		if ( $time eq $ntime ) {
			...
			Wx::MessageBox('Play Music', 'llllll', Wx::wxOK, $main_window);
	...
	$config = [ grep { $_->{status} eq 'enabled' } @$config ];
}
