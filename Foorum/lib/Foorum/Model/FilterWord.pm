...
sub get_data {
    ...
    return wantarray? @{ $cache_value->{value} } : $cache_value->{value} if ($cache_value);
    ...
    foreach my $word (@offensive_words) {
        if ($text =~ /$word/) {
            ...
            foreach my $offset ( 2 .. length($word)) {
                ...
                $converted_chars++; last if ($converted_chars == 2); # that's enough
            }
            substr($asterisk_word, 1, 1) = '*' unless ($asterisk_word =~ /\*/is);
